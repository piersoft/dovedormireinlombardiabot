<?php
/**
* Telegram Bot example for dovedormireinlombardiabot
 ...
 function shell($telegram,$text,$chat_id,$user_id,$location,$reply_to_msg)
{
	date_default_timezone_set('Europe/Rome');
	$today = date("Y-m-d H:i:s");
	if (strpos($text,'@dovedormireinslombardiabot') !== false) $text=str_replace("@dovedormireinslombardiabot ","",$text);

	if ($text == "/start" || $text == "©️ Informazioni") {
		$img = curl_file_create('logo.png','image/png');
		$contentp = array('chat_id' => $chat_id, 'photo' => $img);
		$telegram->sendPhoto($contentp);
		$reply = "Benvenuto. Per ricercare una struttura ricettiva della Regione Lombardia, censita dalla DG Commercio, Turismo e Terziariola, digita il nome del Comune oppure clicca sulla graffetta (📎) e poi 'posizione' . Puoi anche ricercare per parola chiave nel titolo anteponendo il carattere ?. Verrà interrogato il DataBase openData utilizzabile con licenza IoDL2.0 presente su https://dati.lombardia.it/Turismo/Strutture-Ricettive-Alberghiere-e-extra-alberghier/745d-3uyg. In qualsiasi momento scrivendo /start ti ripeterò questo messaggio di benvenuto.\nQuesto bot è stato realizzato da @piersoft e potete migliorare il codice sorgente con licenza MIT che trovate su https://github.com/piersoft/dovedormireinlombardiabot.\nLa propria posizione viene ricercata grazie al geocoder di openStreetMap con Lic. odbl.\nSono state usate le API di Socrata https://dev.socrata.com";
		$content = array('chat_id' => $chat_id, 'text' => $reply,'disable_web_page_preview'=>true);
		$telegram->sendMessage($content);
		$log=$today. ",new chat started," .$chat_id. "\n";
		file_put_contents('/usr/www/piersoft/dovedormireinlombardiabot/db/telegram.log', $log, FILE_APPEND | LOCK_EX);
...
exit;
		...
		$telegram->sendMessage($content);
		$homepage="";
		$json_string=file_get_contents("https://www.dati.lombardia.it/resource/366u-u4cw.geojson?id_struttura=".$text);
		$json_string=strip_tags($json_string);
		$parsed_json = json_decode(utf8_encode($json_string));

		$ciclo=0;
$text=trim($text);
$text=str_replace("\n","",$text);
//$text="387868148"; // debug
...
foreach ($parsed_json->{'features'} as $i => $value) {
	...
	/*
	$content = array('chat_id' => $chat_id, 'text' => $parsed_json->{'features'}[0]->{'properties'}->{'id_struttura'},'disable_web_page_preview'=>true);
	$telegram->sendMessage($content);
		$this->create_keyboard_temp($telegram,$chat_id);
	exit;
*/
